(function () {
  ...
  function getCardNumberForSave() {
    ...
    if (editingId && raw.includes("*")) {
      const existing = methods.find(function (m) {
        return m.id === editingId;
      });
      if (existing)
        return { digits: null, lastFour: existing.lastFour, unchanged: true };
    }
  ...
  function buildCardHtml(method) {
    ...
    return (
      ...
      '"' +
      ' data-billing="' +
      escapeAttr(method.billingAddress) +
      '"' +
      ' data-last-four="' +
      method.lastFour +
      '">' +
      ...
      '<span class="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-amber-600 to-stone-800 text-xs font-bold text-white">' +
      typeIcon(method.cardType) +
      "</span>" +
      "<div>" +
      '<p class="text-sm font-semibold text-stone-900">' +
      typeLabel(method.cardType) +
      "</p>" +
  ...
  function populateFormFromCard(cardEl) {
    ...
    const radio = document.querySelector(
      'input[name="cardType"][value="' + type + '"]',
    );
  ...
  form.addEventListener("submit", function (event) {
    ...
    if (editingId) {
      const index = methods.findIndex(function (m) {
        return m.id === editingId;
      });
  ...
  document
    .getElementById("scrollToFormBtn")
