...
public sealed class BuyNowSellE2ETests : E2ETestBase
{
    ...
    {
        Go("/BuyNow");
        var cards = Driver.FindElements(By.CssSelector("[data-id], a[href*='/BuyNow/Detail/']"));
        if (cards.Count == 0) return;
        var href = cards[0].GetAttribute("href");
    ...
    {
        Go("/BuyNow");
        Assert.True(Driver.FindElements(By.CssSelector("button[data-auth-tab='login']")).Count > 0);
    }
