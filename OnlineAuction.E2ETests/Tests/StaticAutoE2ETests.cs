...
public sealed class StaticAutoE2ETests : E2ETestBase
{
    ...
    {
        Go("/Contact");
        Assert.True(Driver.FindElements(By.CssSelector("form, .contact, main")).Count > 0);
    }
    ...
    {
        Go("/");
        var links = Driver.FindElements(By.CssSelector("a[href*='Language/SetCulture'], form[action*='Language']"));
