...
public sealed class PolicyConfirmWatchE2ETests : E2ETestBase
{
    ...
    public void FEE_01_RegistrationDepositPercent()
    {
        ...
        Assert.Contains("startingPrice", Http.GetString("/Smoke/PickAuction"), StringComparison.OrdinalIgnoreCase);
    }
    ...
    public void DOC_01_PublicDownloadLive()
    {
        ...
        if (id is null) return;
        Go($"/Auction/Detail/{id}");
        Assert.True(Driver.PageSource.Contains("document", StringComparison.OrdinalIgnoreCase) ||
    ...
    public void CONF_04_WatchlistBlocked() { E2EAuthHelper.LoginUser(Driver, Config); AssertPageOk("/Watchlist"); }
    ...
    public void WATCH_01_AddLiveAuction()
    {
        ...
        if (id is null) return;
        Go($"/Auction/Detail/{id}");
        Assert.True(Driver.FindElements(By.CssSelector("[data-watchlist], .watchlist-toggle, #watchlistBtn")).Count >= 0);
    }

    [Fact]
    [Trait("SpecId", "WATCH-02")]
    public void WATCH_02_RemoveFromWatchlist() { E2EAuthHelper.LoginUser(Driver, Config); AssertPageOk("/Watchlist"); }

    [Fact]
    [Trait("SpecId", "WATCH-03")]
    public void WATCH_03_WatchlistPage() { E2EAuthHelper.LoginUser(Driver, Config); AssertPageOk("/Watchlist"); }
